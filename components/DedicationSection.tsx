...
export default function DedicationSection({
      ...
      <section className="relative min-h-[100svh] w-full flex flex-col justify-center items-center overflow-hidden bg-[#131313] py-12 sm:py-16 px-3 sm:px-6">
        <div className="w-full max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 items-center z-10">
          ...
          <div className="md:col-span-4 text-center space-y-5 p-5 sm:p-8 rounded-3xl bg-black/60 backdrop-blur-xl border border-rosegold/30 shadow-2xl">
            ...
            <h2 className="font-serif text-xl sm:text-2xl text-white font-light">Con la Bendición de Mis Padres</h2>
            <div className="w-10 h-[1px] bg-rosegold/40 mx-auto" />
            <div className="text-gray-200 font-serif text-base sm:text-lg leading-relaxed">
              {parents.mother} <br />
            ...
            {parents.godparents && (
              <div className="pt-2">
                ...
                <p className="font-serif text-xs sm:text-sm text-gray-300">{parents.godparents}</p>
              </div>
            )}
          </div>
  ...
  if (layout === 'floating-glass') {
    return (
      <section className="relative min-h-[100svh] w-full flex flex-col justify-center items-center overflow-hidden bg-[#131313] py-12 sm:py-16 px-3 sm:px-4">
        ...
        <div className="relative z-10 w-full max-w-2xl rounded-3xl bg-black/60 backdrop-blur-2xl border border-rosegold/30 p-6 sm:p-12 text-center space-y-5 sm:space-y-6 shadow-2xl">
          <div className="w-20 h-20 sm:w-24 sm:h-24 mx-auto rounded-full overflow-hidden border-2 border-rosegold shadow-xl">
            <img src={backgroundImage} alt="" className="w-full h-full object-cover" />
          </div>
          ...
          <p className="font-serif text-xl sm:text-3xl text-gray-100 font-normal leading-relaxed">
            {parents.mother} <br />
            <span className="text-xs sm:text-sm font-sans font-light text-rosegold block my-1">&</span>
            {parents.father}
          </p>

          {parents.godparents && (
            <div className="space-y-1 pt-3">
              <span className="text-[9px] sm:text-[10px] tracking-widest uppercase text-rosegold font-sans block">Mis Padrinos</span>
              <p className="font-serif text-base sm:text-lg text-gray-200">{parents.godparents}</p>
            </div>
          )}
        </div>
      </section>
  ...
  return (
    <section className="relative min-h-[100svh] w-full flex flex-col justify-center items-center overflow-hidden bg-[#131313] py-16 sm:py-20 px-4 sm:px-6">
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto text-center space-y-6 sm:space-y-10">
        <motion.div
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 sm:px-4 sm:py-1.5 rounded-full bg-black/40 border border-rosegold/30 text-rosegold text-[10px] sm:text-[11px] tracking-widest uppercase backdrop-blur-md">
            <Sparkles className="w-3 h-3 text-rosegold" />
            <span>Nuestra Familia</span>
          </div>

          <h2 className="font-serif text-2xl sm:text-5xl text-white font-light drop-shadow-[0_4px_25px_rgba(0,0,0,0.9)]">
            Con la Bendición de Mis Padres
          </h2>
          ...
          <div className="pt-2 sm:pt-4 space-y-1 sm:space-y-2">
            <p className="font-serif text-xl sm:text-4xl text-gray-100 font-normal leading-relaxed drop-shadow-[0_4px_20px_rgba(0,0,0,0.9)]">
              {parents.mother} <br />
              <span className="text-sm sm:text-base font-sans font-light text-rosegold block my-1">&</span>
              {parents.father}
            </p>
          </div>
          ...
          {parents.godparents && (
            <div className="space-y-1 pt-4">
              <span className="text-[9px] sm:text-xs tracking-widest uppercase text-rosegold-light/80 font-sans font-light block">
                Y de mis queridos padrinos
              </span>
              <p className="font-serif text-lg sm:text-2xl text-gray-200 font-light drop-shadow-md">
                {parents.godparents}
              </p>
            </div>
          )}

          <div className="pt-4 flex items-center justify-center gap-2 text-rosegold/80">
            <Heart className="w-4 h-4 sm:w-5 sm:h-5 fill-current text-rosegold animate-pulse" />
          </div>
        </motion.div>
      </div>
    </section>
